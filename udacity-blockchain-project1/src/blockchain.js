...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      //   resolve(block);
      // } else {
      //   reject(Error("Error adding a Block."));
      // }
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (messageTime > currentTime - 5 * 60) {
        ...
        if (is_valid) {
          ...
          reject(Error("ERR: Not valid signature."));
        }
      } else {
        reject(
          Error("ERR: A new star can be added every five minutes or less.")
        );
      }
    });
  }
  ...
  validateChain() {
    ...
    //       if (!validation) {
    //         console.log("ERR: ERROR VALIDATING DATA");
    //       } else if (block.previousBlockHash != self.chain[i - 1].hash) {
    //         console.log("ERR: ERROR WITH PREVIOUS BLOCK HASH");
    //       }
    //     }
    //     if (errorLog) {
    //       resolve(errorLog);
    ...
    //     }
    //   } else {
    //     reject(Error("ERR: Cannot validate chain.")).catch((error) => {
      ...
      self.chain.forEach((block, index) => {
        if (block.height > 0) {
          ...
          if (block.previousBlockHash !== previousBlock.hash) {
            const errorMessage = `ERR: Block ${index} has different hash ${block.previousBlockHash}, != ${previousBlock.hash}`;
