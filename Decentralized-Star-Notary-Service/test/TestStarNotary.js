...
it("can add the star name and star symbol properly", async () => {
  ...
  await instance.createStar("Awesomer Star!", tokenId, { from: owner1 });

  const tokenName = await instance.name();
  const tokenSymbol = await instance.symbol();

  assert.equal(tokenName, "KOKOS PAPA TOKEN");
  assert.equal(tokenSymbol, "KPT");
});
...
it("lets 2 users exchange stars", async () => {
  ...
  const tokenId1 = 21;
  const owner1 = accounts[1];
  await instance.createStar("Awesomer Star!", tokenId1, { from: owner1 });

  const tokenId2 = 22;
  const owner2 = accounts[2];
  await instance.createStar("Awesomer Star!!", tokenId2, { from: owner2 });
