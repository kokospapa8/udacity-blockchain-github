...
contract FlightSuretyApp {
    ...
    modifier requireAirlineFunded() {
        require(
            dataContract.isFunded(msg.sender),
    ...
    {
        require(
            dataContract.airlineExists(airline),
        ...
        if (registeredAirlines.length < 5) {
            ...
            require(
                !dataContract.isVoted(msg.sender, airline),
                "Requires registering airline hasn't already voted"
            );

            uint256 votes = dataContract.voteAirline(msg.sender, airline);
            if (
                SafeMath.div(
    ...
    function FundAirline() external payable requireIsOperational {
        require(!dataContract.isFunded(msg.sender), "funding already provided");
        require(
            msg.value == 10 ether,
    ...
    function registerFlight(
        ...
        dataContract.registerFlight(
            getFlightKey(airlineAddress, flight, timestamp),
            airlineAddress,
            flight,
            timestamp
        );
     ...
     * if flight is 20, reimburse, if not 20 do nothing
     */
    function processFlightStatus(
        ...
        if (statusCode == STATUS_CODE_LATE_AIRLINE) {
            dataContract.creditInsurees(airline, flight, CREDIT_MULTIPLIER);
        }
    }
     ...
     * @dev buy insurance
     */
    function buyInsurance(address airline, string flight)
        external
        payable
        requireIsOperational
    {
